...
#######################################################################################
#
#   puts >>>[EMail::Init MelHeravi@Gmail.com http://www.Melify.com smtpout.secureserver.net]
#   EMail::Send MelHeravi@Gmail.com "" "" "Testing EAMIL" "Hello World"
#
...
proc ::EMail::Finish { token {errormsg ""} } {
    ...
    if {$debug} then {
	puts "Finish: $token";flush stdout;
    }
...
proc ::EMail::Event {token} {
	...
	if {$ReceiveCode == "$state(GoodCode)"} then {
	    ...
	    if {$debug} then {
		puts "Event: Expected $state(GoodCode) but got $ReceiveCode";flush stdout;
	    }
	    set Process Bad
	}
	switch $state(Status) {
		...
		if {"$Process" == "Bad"} then {
		    ...
		    if {$debug} then {
			puts "Event: Status now $state(Status)";flush stdout;
		    }
		...
		if {"$Process" == "Bad"} then {
		    ...
		    if {$debug} then {
			puts "Event: Status now $state(Status)";flush stdout;
			puts "Event: RCPT <[lindex $state(RCPT) 0]>";flush stdout;
		    }
		    set state(GoodCode) "250"
		...
		if {"$Process" == "Bad"} then {
			...
			foreach file $state(-files) {
				...
				if {$debug} then {
				    puts "Attachment: $file ([file size "$file"])";flush stdout;
				}
				base64 -mode encode -in $fHandle -out $state(sock)
				close $fHandle
			    } else {
				if {$debug} then {
				    puts "** Attachment fail: $file";flush stdout;
				}
			    }
			}      
			puts $state(sock) "--$state(Boundry)--\n"
...
proc ::EMail::Wait { EMailToken } {
	...
	if {[llength $EMailCompleteList] == 0} {
	    if {$debug} then {
		puts "0: [clock seconds] Wait: Waiting for Any";flush stdout;
	    }
	...
	if {(![info exist EMailWaitFlags($EMailToken)]) || ($EMailWaitFlags($EMailToken) != 1)} {
	    if {$debug} then {
		puts "1: [clock seconds] Wait: Waiting for $EMailToken";flush stdout;
	    }
