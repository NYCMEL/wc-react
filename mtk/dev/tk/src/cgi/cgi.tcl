...
proc cgi_url {display args} {
    ...
    if {[lindex $args 0] == "#"} {
	set args [lreplace $args 0 0 "javascript:void(0)"]
    }
...
proc cgi_img {args} {
    ...
    set buf "<img src=\"http://$::env(HTTP_HOST)[lindex $args 0]\""
    foreach a [lrange $args 1 end] {
...
proc cgi_bold	    {s} {return "<em>$s</em>"}
...
proc cgi_body {args} {
	...
	puts ">>>$errMsg $savedInfo $savedCode"
    }
    _cgi_body_end
}
...
proc _cgi_body_start {args} {
    ...
    cgi_debug {
	global env
	catch {cgi_puts "<pre class=cgi-debug>Input:<BR>$_cgi(input)</pre>"}
	catch {cgi_puts "<pre class=cgi-debug>Cookie:<BR>$env(HTTP_COOKIE)</pre>"}
    }
...
proc cgi_javascript {args} {
    # cgi_puts "<script[_cgi_lrange $args 0 [expr [llength $args]-2]] type='text/javascript'>"
    # cgi_puts "<!--- Hide script from browsers that dont understand JavaScript"
...
proc cgi_export {nameval} {
    ...
    cgi_put "<input type=hidden name=\"$name\" value=[cgi_dquote_html $value]></input>"
}
...
proc cgi_file_button {name args} {
    ...
    cgi_put "<input type=file name=\"$name\"[_cgi_list_to_string $args]></input>"
}
...
proc cgi_table_head {args} {
    cgi_put "<thead"
    _cgi_close_proc_push "cgi_put </thead>"
...
proc cgi_table_body {args} {
    cgi_put "<tbody"
    _cgi_close_proc_push "cgi_put </tbody>"
...
cgi_admin_mail_addr "MelifyIT@gmail.com";# you should override this!
