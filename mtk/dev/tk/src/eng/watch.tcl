...
##### 
##############################################
proc tk::watch {varname key op} {
    global $varname
    
    if {[string index $varname 0] == ":"} {
	set x [string range $varname 2 end]
    } else {
	set x $varname
    }
    
    set v [string trim [lstring [set ${varname}($key)]]]

    switch $op {
	"w" {
	    put "<script>jQuery.cookie('${x}($key)','$v')</script>"
	}
	"u" {
	    put "<script>jQuery.removeCookie('${x}($key)')</script>"
	}
    }
}
